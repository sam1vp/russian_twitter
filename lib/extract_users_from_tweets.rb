...
def extract_user(tweet)
  ...
  if !TwitterAccount.find_by_screen_name(name) and !Username.find_by_username(name)
...
tweet_set = Tweet.find(:all, :include => :language, :conditions => "languages.iso_lang_code = 'ru'")
tweet_set.each do |tweet|
  if !!tweet.tags and tweet.language
    extract_user(tweet)
  end
